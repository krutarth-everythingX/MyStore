...
export const Profile = () => {
  const { user, token, logout, updateProfile, verifyEmailCode, resendVerificationCode } = useAuth();
  ...
  const [rvFetched, setRvFetched] = useState(false);
  ...
  // Lazy-fetch orders only once when section opens
  useEffect(() => {
    if (openSection === 'orders' && !ordersFetched && token) {
      setOrdersLoading(true);
      fetch(`${API_BASE}/orders`, {
        headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
      })
        .then((r) => r.ok ? r.json() : [])
        .then((d) => { setOrders(Array.isArray(d) ? d : []); setOrdersFetched(true); })
        .catch(() => { setOrders([]); setOrdersFetched(true); })
        .finally(() => setOrdersLoading(false));
    }
  }, [openSection, ordersFetched, token]);

  // Lazy-fetch recently viewed
  useEffect(() => {
    if (openSection === 'recently-viewed' && !rvFetched && token) {
      setRvLoading(true);
      fetch(`${API_BASE}/recently-viewed`, {
        headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
      })
        .then((r) => r.ok ? r.json() : [])
        .then((d) => { setRecentlyViewed(Array.isArray(d) ? d : []); setRvFetched(true); })
        .catch(() => { setRecentlyViewed([]); setRvFetched(true); })
        .finally(() => setRvLoading(false));
    }
  }, [openSection, rvFetched, token]);
            ...
            <Section id="wishlist" icon={Heart} label="Wishlist" badge={wishlist.length}>
              {wishlist.length === 0 ? (
                <div className="pv-empty-state">
                  ...
                  <Link to="/"><Button variant="primary">Browse Products</Button></Link>
                </div>
              ) : (
                <div className="pv-product-grid">
                  {wishlist.map((item) => {
                    ...
                    return (
                      <Link key={item.id} to={`/products/${prod.id}`} className="pv-product-tile">
            ...
            <Section id="recently-viewed" icon={Clock} label="Recently Viewed">
              {rvLoading ? (
                ...
                <div className="pv-empty-state">
                  ...
                  <Link to="/"><Button variant="primary">Go Shopping</Button></Link>
                </div>
              ) : (
                <div className="pv-product-grid">
