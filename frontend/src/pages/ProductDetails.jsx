...
// Reusable Star Rating visual element
...
const ProductRecommendations = ({ productId, categoryId }) => {
  ...
  useEffect(() => {
    if (productId) {
      // Fetch related items by category (we fetch all products and filter locally for robust sandbox)
    ...
    <div style={{ marginTop: 40 }}>
      ...
      <div style={{
        ...
        {related.map((p) => {
          ...
          return (
            <Link key={p.id} to={`/products/${p.id}`} className="card" style={{ textDecoration: 'none', padding: 16, display: 'flex', flexDirection: 'column', height: '100%' }}>
              <div style={{
                height: 140,
...
export const ProductDetails = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/products/${id}`)
      ...
      .then(data => {
        setProduct(data);
        // Track recently viewed if logged in as buyer
        if (token && user?.role === 'buyer') {
          fetch(`${API_BASE}/recently-viewed/${data.id}`, {
            method: 'POST',
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || 'Verified customer check failed.');
      }
  ...
  const price = isSale ? currentProduct.sale_price : currentProduct.regular_price;
  ...
  return (
    <div className="buyer-layout">
      ...
      <main className="container details-main animate-fade-in">
        <Link to="/" className="details-back-link label-md">
          <ArrowLeft size={16} style={{ marginRight: 6 }} />
          Back to Catalog
        </Link>
        ...
        {/* Extended Description and Warehouse Stock details */}
            ...
            {token && user?.role === 'buyer' ? (
              ...
              <Card title="Feedback & Review">
                <p className="body-md" style={{ color: 'var(--color-outline)', lineHeight: '20px' }}>
                  Please <Link to="/login" style={{ color: 'var(--color-primary)', textDecoration: 'underline' }}>log in</Link> as a buyer to leave verified customer feedback on items you purchase.
                </p>
              </Card>
            )}
          </div>
        ...
        {product && (
          <ProductRecommendations productId={product.id} categoryId={product.categories?.[0]?.id} />
        )}
      </main>
      <Footer />
    </div>
