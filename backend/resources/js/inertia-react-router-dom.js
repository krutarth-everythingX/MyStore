...
export function Link({
  ...
  return <InertiaLink href={href ?? to ?? '#'} {...props}>
      {children}
    </InertiaLink>;
}
...
export function useSearchParams() {
  ...
  const setSearchParams = nextValue => {
    const nextParams = nextValue instanceof URLSearchParams ? new URLSearchParams(nextValue.toString()) : new URLSearchParams(nextValue);
    const query = nextParams.toString();
    router.get(`${url.pathname}${query ? `?${query}` : ''}`, {}, {
      replace: true,
      preserveScroll: true,
      preserveState: false
    });
